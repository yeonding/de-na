export default class Player {
    ...
    constructor(selectedCharacter, canvas) {
    ...
    // 플레이어가 몬스터를 공격하는 함수
    autoAttack(monsters, musicsound) {
    ...
    draw(selectedCharacter, characterDirection, ctx, canvas) {
        let speed = 1;
        ...
        if (this.position.x >= canvas.width - 40) {
        ...
        if (this.position.y >= canvas.height - 45) {
            this.position.y = canvas.height - 45
        }
    }
}